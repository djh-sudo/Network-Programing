    ...
    , ui(new Ui::Server)
{
    ...
    inicialUserTabel();
    inicialUserGroup();
    qDebug()<<db->allFriendData("2122263");
        ...
        connect(socket, &QTcpSocket::readyRead,[=](){
            ...
            for(unsigned int i = 0;i<content.size();){
                if(content[i] == USER_LOGIN){//处理登录请求
                    QString s = QString(buffer).section("##",0,i+3);
                    UserLogin(s,socket);
                    i = i + 3;
                }
                else if(content[i] == USER_REGISTER){//处理注册请求
                    QString s =QString(buffer).section("##",0,i+4);
                    ...
                    QString s = QString(buffer).section("##",0,i + number + 4);
                    if(addComponent(s)){
